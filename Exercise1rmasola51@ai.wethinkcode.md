 ...
 Part 4: Reflection and Presentation
      Briefly explaination of the TaskManager application.

      The application is a Python command-line program. 
      Its main parts are `cli.py` for user commands, 
      task_manager.py for the main logic,
      models.py for tasks, and 
      storage.py for saving and loading data from tasks.json.

      The three main features are:

      Task creation: The user creates a task through the CLI, and it is saved to tasks.json.
      Prioritization: Each task has a priority that helps determine how important it is.
      Completion:The TaskManager finds the task and calls mark_as_done(). The status changes to DONE, 
      and the completion time is recorded before the task is saved.

      One interesting design approach is separation of responsibilities. 
      Each file has a specific job, which makes the application easier to understand and maintain.

      The most challenging part was following how data moves between different files.
      The prompts helped me by guiding me to trace the data flow step by step instead of looking at each file separately.

      Overall, I learned how the different parts of the application work together to manage and save tasks.
